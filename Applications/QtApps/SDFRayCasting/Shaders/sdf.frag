...
vec3 render(in vec3 ro, in vec3 rd)
{
    ...
    if(m > -0.5) {
        ...
        if(m < 1.5) {
            float f = 0.0;//checkersGradBox( 5.0*pos.xz );
            col = 0.3 + f * vec3(0.1);
        }
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // camera
    vec3 ro = camPosition.xyz;            // vec3(-0.5 + 3.5, 1.0, 0.5);
    vec3 ta = camPosition.xyz + u_RayDir; //vec3(-0.5, -0.4, 0.5);
...
void main()
{
    mainImage(outColor, vec2(gl_FragCoord));
}

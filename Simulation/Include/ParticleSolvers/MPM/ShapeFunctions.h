...
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
inline Matrix33 dev(const Matrix33& m)
{
    return Matrix33(m - I3(m.trace() / 3.));
}

inline Vector3 inner(const Vector3& vec, const Matrix33& mat)
{
    return Vector3(vec[0] * mat(0, 0) + vec[1] * mat(1, 0) + vec[2] * mat(2, 0),
                   vec[0] * mat(0, 1) + vec[1] * mat(1, 1) + vec[2] * mat(2, 1),
                   vec[0] * mat(0, 2) + vec[1] * mat(1, 2) + vec[2] * mat(2, 2));
}

inline Matrix33 inner(const Matrix33& m1, const Matrix33& m2)
{
    Matrix33 tmp;
    tmp << m1(0, 0) * m2(0, 0) + m1(0, 1) * m2(1, 0) + m1(0, 2) * m2(2, 0),

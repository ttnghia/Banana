    ...
    while(frameTime < m_GlobalParams->frameDuration)
    {
        m_Logger->printRunTime("Sub-step time: ", subStepTimer,
                               [&]()
                               {
                                   ...
                                   m_Logger->printRunTime("Move particles: ", funcTimer, [&]() { updateParticles(substep); });
...
{
    initializeMass();
    initializeVelocities(timestep);

    explicitVelocities(timestep);
    if(m_SimParams->implicitRatio > Tiny)
        implicitVelocities(timestep);
                                          ...
                                          for(Int idx = 0, y = oy - 1, y_end = y + 3; y <= y_end; y++)
                                          {
                                              for(Int x = ox - 1, x_end = x + 3; x <= x_end; x++, idx++)
                                              {
                                                  ...
                                                  if(gridData().imp_active(x, y))
                                                  {
                                                      ...
                                                      gridData().force(x, y) += deltaForce(p, gridData().r(x, y), particleData().weightGradients[p * 16 + idx], timestep);
                                                  }
                                                  gridData().nodeLocks(x, y).unlock();
                                              }
                                          }
    ...
    ParallelFuncs::parallel_for<UInt>(0, m_Grid.getNumNodeX(),
                                      ...
                                      [&](UInt x, UInt y)
                                      {
                                          ...
                                          if(gridData().active(x, y))
                                          {
                                              ...
                                              for(int i = 0; i < solverDimension(); ++i)
                                              {
...
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
void MPM2DSolver::updateGradient(Real timestep)
{
    ParallelFuncs::parallel_for<UInt>(0, getNumParticles(),
                                      [&](UInt p)
                                      {
                                          ...
                                          velGrad *= timestep;
                                          TensorHelpers::sumToDiag(velGrad, Real(1.0));
...
void MPM2DSolver::applyPlasticity()
{
    ParallelFuncs::parallel_for<UInt>(0, getNumParticles(),
                                      [&](UInt p)
                                      {
                                          ...
                                          TensorHelpers::diagProduct(polar_s, svd_e);
...
Mat2x2r MPM2DSolver::computeEnergyDerivative(UInt p)
{
    ...
    TensorHelpers::sumToDiag(temp, m_SimParams->lambda * Je * (Je - Real(1.0)));
    //Add hardening and volume
    return particleData().volumes[p] * harden * temp;
}

//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
Vec2r MPM2DSolver::deltaForce(UInt p, const Vec2r& u, const Vec2r& weight_grad, Real timestep)
{
    ...
    if(TensorHelpers::maxAbs<Real>(del_elastic) < Tiny)
        return Vec2r(0);
    ...
    Mat2x2r cofactor = TensorHelpers::cofactor(elasticDeformGrad);
    ...
    Mat2x2r del_cofactor = TensorHelpers::cofactor(del_elastic);
    ...
    cofactor     *= TensorHelpers::frobeniusInnerProduct<Real>(cofactor, del_elastic);

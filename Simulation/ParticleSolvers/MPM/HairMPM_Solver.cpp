...
void HairMPM_Solver<N, RealType>::allocateSolverMemory() {
    ...
    this->m_MPMParams    = std::static_pointer_cast<MPM_Parameters<N, RealType>>(m_HairMPMParams);
    this->m_SolverParams = std::static_pointer_cast<SimulationParameters<N, RealType>>(m_HairMPMParams);

    m_HairMPMData      = std::make_shared<HairMPM_Data<N, RealType>>();
    this->m_MPMData    = std::static_pointer_cast<MPM_Data<N, RealType>>(m_HairMPMData);
    this->m_SolverData = std::static_pointer_cast<SimulationData<N, RealType>>(m_HairMPMData);
...
void HairMPM_Solver<N, RealType>::generateParticles(const JParams& jParams) {
    ...
    ////////////////////////////////////////////////////////////////////////////////
    solverData().classifyParticles(this->m_SolverParams);
    solverData().find_d0(this->m_SolverParams);
...
void HairMPM_Solver<N, RealType>::advanceVelocity(RealType timestep) {
    this->m_Logger->printRunTime("{   Reset grid data", [&]() { gridData().resetGrid(); });
    this->m_Logger->printRunTimeIndent("Map particle masses to grid", [&]() { this->mapParticleMasses2Grid(); });
    this->m_Logger->printRunTimeIndentIf("Compute particle volumes", [&]() { return this->initParticleVolumes(); });
    this->m_Logger->printRunTimeIndent("Map particle velocities to grid", [&]() { this->mapParticleVelocities2Grid(timestep); });
    this->m_Logger->printRunTimeIndent("Damp velocity",                   [&]() { computeDamping(); });

    if(solverParams().implicitRatio < Tiny<RealType>()) {
        this->m_Logger->printRunTimeIndent("Velocity explicit integration", [&]() { explicitIntegration(timestep); });
    } else {
        this->m_Logger->printRunTimeIndent("Velocity implicit integration", [&]() { this->implicitIntegration(timestep); });
    }

    this->m_Logger->printRunTimeIndent("Constrain grid velocity",          [&]() { this->gridCollision(timestep); });
    this->m_Logger->printRunTimeIndent("Map grid velocities to particles", [&]() { this->mapGridVelocities2Particles(timestep); });
    this->m_Logger->printRunTimeIndent("Update particle states",           [&]() { updateParticleStates(timestep); });
    //this->m_Logger->printRunTimeIndent("Compute plasticity",               [&]() { computePlasticity(); });
}
...
template class HairMPM_Solver<3, Real>;
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+

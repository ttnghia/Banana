...
public:
	...
	template<class T>
	void append(const Vector<Vector<T> >& vData, bool bWriteVectorSize = true)
	{
		if(bWriteVectorSize) {
			const UInt numElements = static_cast<UInt>(vData.size());
			append<UInt>(numElements);
	...
	template<class T>
	size_t getData(Vector<Vector<T> >& vData, size_t startOffset = 0, UInt vSize = 0)
	{
		size_t segmentStart = startOffset;
		size_t segmentSize  = 0;
		UInt   numElements  = vSize;
...
public:
	...
	static String byteToHex(const T* arrBytes, size_t arrSize)
	{
		...
		for(size_t i = 0; i < arrSize; ++i) {
			ss << std::hex << std::uppercase << std::setw(3) << static_cast<int>(arrBytes[i]);
		}

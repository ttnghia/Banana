...
public:
    ...
    template <typename T> explicit Point(const Point<T, N> &p)
    {
    ...
    template <int M> explicit Point(const Point<TYPE, M> &p)
    {
        ...
        {
            CY_MEMCOPY(TYPE, data, p.data, M); CY_MEMCLEAR(TYPE, data, N - M);
        }
    }
    template <typename T, int M> explicit Point(const Point<T, M> &p)
    {
        ...
        {
            CY_MEMCONVERT(TYPE, data, p.data, M); CY_MEMCLEAR(TYPE, data, N - M);
        }
    }
    ...
    template <typename P> explicit Point(const P &p)
    {
        for(int i = 0; i < N; ++i) data[i] = (TYPE)p[i];
    }
    ...
    template <int M> void CopyData(TYPE *p)
    {
        if(M <= N) CY_MEMCOPY(TYPE, p, data, M); else
        {
            CY_MEMCOPY(TYPE, p, data, N); CY_MEMCLEAR(TYPE, p + N, M - N);
        }
    }
    template <typename T, int M> void ConvertData(T *p)
    {
        if(M <= N) CY_MEMCONVERT(T, p, data, M); else
        {
            CY_MEMCONVERT(T, p, data, N); CY_MEMCLEAR(T, p + N, M - N);
        }
    }
    ...
    {
        Point p = operator*(*this); return p.Sum();
    ...
    TYPE  Sum() const
    {
        TYPE v = data[0]; for(int i = 1; i < N; ++i) v += data[i]; return v;
    }		//!< Returns the sum of its components
    bool  IsZero() const
    {
        for(int i = 0; i < N; ++i) if(data[i] != TYPE(0)) return false; return true;
    }	//!< Returns true if all components are exactly zero
    TYPE  Min() const
    {
        TYPE m = data[0]; for(int i = 1; i < N; ++i) if(m > data[i]) m = data[i]; return m;
    }
    TYPE  Max() const
    {
        TYPE m = data[0]; for(int i = 1; i < N; ++i) if(m < data[i]) m = data[i]; return m;
    ...
    int   MinID() const
    {
        TYPE m = data[0]; int ix = 0; for(int i = 1; i < N; ++i) if(m > data[i])
        {
            m = data[i]; ix = i;
        } return m;
    }
    ...
    {
        ClampMin(minValue); ClampMax(maxValue);
    }
    void ClampMin(const TYPE &v)
    {
        for(int i = 0; i < N; ++i) data[i] = (data[i] < v) ? v : data[i];
    }
    ...
    void Abs()
    {
        for(int i = 0; i < N; i++) data[i] = cyAbs(data[i]);
    }	//!< Converts all negative components to positive values

//!@name Unary operators
    Point operator - () const
    {
        Point r; for(int i = 0; i < N; ++i) r.data[i] = -data[i]; return r;
    }

    //!@name Binary operators
    Point operator + (const Point &p) const
    {
        Point r; for(int i = 0; i < N; ++i) r.data[i] = data[i] + p.data[i]; return r;
    }
    Point operator - (const Point &p) const
    {
        Point r; for(int i = 0; i < N; ++i) r.data[i] = data[i] - p.data[i]; return r;
    ...
    {
        Point r; for(int i = 0; i < N; ++i) r.data[i] = data[i] + v; return r;
    }
    Point operator - (const TYPE  &v) const
    {
        Point r; for(int i = 0; i < N; ++i) r.data[i] = data[i] - v; return r;
    ...
    {
        CY_MEMCOPY(TYPE, data, p.data, N); return *this;
    }
    const Point& operator += (const Point &p)
    {
        for(int i = 0; i < N; ++i) data[i] += p.data[i]; return *this;
    }
    const Point& operator -= (const Point &p)
    {
        for(int i = 0; i < N; ++i) data[i] -= p.data[i]; return *this;
    ...
    {
        for(int i = 0; i < N; ++i) data[i] += v; return *this;
    }
    const Point& operator -= (const TYPE  &v)
    {
        for(int i = 0; i < N; ++i) data[i] -= v; return *this;
    ...
    {
        for(int i = 0; i < N; ++i) if(data[i] != p.data[i]) return false; return true;
    }
    bool operator != (const Point& p) const
    {
        for(int i = 0; i < N; ++i) if(data[i] != p.data[i]) return true; return false;
    }
    ...
    {
        Point r = operator*(p); return r.Sum();
    }	//!< Dot product
...
public:
    ...
    template <int M> explicit Point2(const Point<TYPE, M> &p)
    {
        p.CopyData<2>(&x);
    }
    template <typename T, int M> explicit Point2(const Point<T, M> &p)
    {
        p.ConvertData<TYPE, 2>(&x);
    }
...
//!@name General methods
    TYPE   LengthSquared() const
    {
        Point2 p = operator*(*this); return p.Sum();
    }	//!< Returns the square of the length. Effectively, this is the dot product of the vector with itself.
    TYPE   Length() const
    {
        return (TYPE)cySqrt(LengthSquared());
...
//!@name Unary operators
    ...
    {
        Point2 r; r.x = -x; r.y = -y; return r;
    }

    //!@name Binary operators
    Point2 operator + (const Point2 &p) const
    {
        Point2 r; r.x = x + p.x; r.y = y + p.y; return r;
    }
    Point2 operator - (const Point2 &p) const
    {
        Point2 r; r.x = x - p.x; r.y = y - p.y; return r;
    ...
    {
        Point2 r; r.x = x + v;   r.y = y + v;   return r;
    }
    Point2 operator - (const TYPE   &v) const
    {
        Point2 r; r.x = x - v;   r.y = y - v;   return r;
    ...
    {
        Point2 r = operator*(p); return r.Sum();
    }	//!< Dot product
    TYPE operator % (const Point2 &p) const
    {
        return Dot(p);
...
public:
    ...
    template <int M> explicit Point3(const Point<TYPE, M> &p)
    {
        p.CopyData<3>(&x);
    }
    template <typename T, int M> explicit Point3(const Point<T, M> &p)
    {
        p.ConvertData<TYPE, 3>(&x);
    }
...
//!@name General methods
    TYPE   LengthSquared() const
    {
        Point3 p = operator*(*this); return p.Sum();
    }		//!< Returns the square of the length. Effectively, this is the dot product of the vector with itself.
    TYPE   Length() const
    {
        return (TYPE)cySqrt(LengthSquared());
    ...
    void Abs()
    {
        x = cyAbs(x); y = cyAbs(y); z = cyAbs(z);
    }	//!< Converts all negative components to positive values

//!@name Unary operators
    Point3 operator - () const
    {
        Point3 r; r.x = -x; r.y = -y; r.z = -z; return r;
    }

    //!@name Binary operators
    Point3 operator + (const Point3 &p) const
    {
        Point3 r; r.x = x + p.x; r.y = y + p.y; r.z = z + p.z; return r;
    }
    Point3 operator - (const Point3 &p) const
    {
        Point3 r; r.x = x - p.x; r.y = y - p.y; r.z = z - p.z; return r;
    ...
    {
        Point3 r; r.x = x + v;   r.y = y + v;   r.z = z + v;   return r;
    }
    Point3 operator - (const TYPE   &v) const
    {
        Point3 r; r.x = x - v;   r.y = y - v;   r.z = z - v;   return r;
    ...
    {
        Point3 r = operator*(p); return r.Sum();
    }					//!< Dot product
...
public:
    ...
    {}
    template <int M> explicit Point4(const Point<TYPE, M> &p)
    {
        p.CopyData<4>(&x);
    }
    template <typename T, int M> explicit Point4(const Point<T, M> &p)
    {
        p.ConvertData<TYPE, 4>(&x);
    }
...
//!@name General methods
    TYPE   LengthSquared() const
    {
        Point4 p = operator*(*this); return p.Sum();
    }	//!< Returns the square of the length. Effectively, this is the dot product of the vector with itself.
    TYPE   Length() const
    {
        return (TYPE)cySqrt(LengthSquared());
    ...
    TYPE   Min() const
    {
        TYPE mxy = x < y ? x : y; TYPE mzw = z < w ? z : w; return mxy < mzw ? mxy : mzw;
    }
    TYPE   Max() const
    {
        TYPE mxy = x > y ? x : y; TYPE mzw = z > w ? z : w; return mxy > mzw ? mxy : mzw;
    ...
    int    MinID() const
    {
        int  ixy = x < y ? 0 : 1; int  izw = z < w ? 2 : 3; return (&x)[ixy] < (&x)[izw] ? ixy : izw;
    }
    int    MaxID() const
    {
        int  ixy = x > y ? 0 : 1; int  izw = z > w ? 2 : 3; return (&x)[ixy] > (&x)[izw] ? ixy : izw;
    ...
    void ClampMin(const TYPE &v)
    {
        x = (x < v) ? v : x; y = (y < v) ? v : y; z = (z < v) ? v : z; w = (w < v) ? v : w;
    }
    void ClampMax(const TYPE &v)
    {
        x = (x > v) ? v : x; y = (y > v) ? v : y; z = (z > v) ? v : z; w = (w > v) ? v : w;
    ...
    void Abs()
    {
        x = cyAbs(x); y = cyAbs(y); z = cyAbs(z); w = cyAbs(w);
    }	//!< Converts all negative components to positive values

//!@name Unary operators
    Point4 operator - () const
    {
        Point4 r; r.x = -x; r.y = -y; r.z = -z; r.w = -w; return r;
    }

    //!@name Binary operators
    Point4 operator + (const Point4 &p) const
    {
        Point4 r; r.x = x + p.x; r.y = y + p.y; r.z = z + p.z; r.w = w + p.w; return r;
    }
    Point4 operator - (const Point4 &p) const
    {
        Point4 r; r.x = x - p.x; r.y = y - p.y; r.z = z - p.z; r.w = w - p.w; return r;
    ...
    {
        Point4 r; r.x = x + v;   r.y = y + v;   r.z = z + v;   r.w = w + v;   return r;
    }
    Point4 operator - (const TYPE   &v) const
    {
        Point4 r; r.x = x - v;   r.y = y - v;   r.z = z - v;   r.w = w - v;   return r;
    ...
    {
        Point4 r = operator*(p); return r.Sum();
    }	//!< Dot product
...
template <typename TYPE, int N> Point<TYPE, N>::Point(const Point2<TYPE> &p)
{
    if(N <= 2) CY_MEMCOPY(TYPE, data, &p.x, N); else
    {
        CY_MEMCOPY(TYPE, data, &p.x, 2); CY_MEMCLEAR(TYPE, data, N - 2);
    }
}
template <typename TYPE, int N> Point<TYPE, N>::Point(const Point3<TYPE> &p)
{
    if(N <= 3) CY_MEMCOPY(TYPE, data, &p.x, N); else
    {
        CY_MEMCOPY(TYPE, data, &p.x, 3); CY_MEMCLEAR(TYPE, data, N - 3);
    }
}
template <typename TYPE, int N> Point<TYPE, N>::Point(const Point4<TYPE> &p)
{
    if(N <= 4) CY_MEMCOPY(TYPE, data, &p.x, N); else
    {
        CY_MEMCOPY(TYPE, data, &p.x, 4); CY_MEMCLEAR(TYPE, data, N - 4);
    }
}
template <typename TYPE, int N> template <typename T> Point<TYPE, N>::Point(const Point2<T> &p)
{
    if(N <= 2) CY_MEMCONVERT(TYPE, data, &p.x, N); else
    {
        CY_MEMCONVERT(TYPE, data, &p.x, 2); CY_MEMCLEAR(TYPE, data, N - 2);
    }
}
template <typename TYPE, int N> template <typename T> Point<TYPE, N>::Point(const Point3<T> &p)
{
    if(N <= 3) CY_MEMCONVERT(TYPE, data, &p.x, N); else
    {
        CY_MEMCONVERT(TYPE, data, &p.x, 3); CY_MEMCLEAR(TYPE, data, N - 3);
    }
}
template <typename TYPE, int N> template <typename T> Point<TYPE, N>::Point(const Point4<T> &p)
{
    if(N <= 4) CY_MEMCONVERT(TYPE, data, &p.x, N); else
    {
        CY_MEMCONVERT(TYPE, data, &p.x, 4); CY_MEMCLEAR(TYPE, data, N - 4);
    }
}

...
public:
    ...
    vector_dot_product(const std::vector<ScalarType>& vec1, const std::vector<ScalarType>& vec2) : v1(vec1), v2(vec2), result(0)
    {}

    // splitting constructor
    vector_dot_product(vector_dot_product& vdp, tbb::split) : v1(vdp.v1), v2(vdp.v2), result(0)
    {}
...
public:
    ScalarType result;  // put the result here
    // constructor copies the arguments into local storage
    vecvec_dot_product(const std::vector<VectorType>& vec1, const std::vector<VectorType>& vec2) : v1(vec1), v2(vec2), result(0)
    {}

    // splitting constructor
    vecvec_dot_product(vecvec_dot_product& vdp, tbb::split) : v1(vdp.v1), v2(vdp.v2), result(0)
    {}
...
public:
    ScalarType result;  // put the result here
    // constructor copies the arguments into local storage
    vector_min(const std::vector<ScalarType>& vec) : v(vec), result(1e100)
    {}

    // splitting constructor
    vector_min(vector_min& vdp, tbb::split) : v(vdp.v), result(1e100)
    {}
...
public:
    ScalarType result;  // put the result here
    // constructor copies the arguments into local storage
    vector_max(const std::vector<ScalarType>& vec) : v(vec), result(-1e100)
    {}

    // splitting constructor
    vector_max(vector_max& vdp, tbb::split) : v(vdp.v), result(-1e100)
    {}
...
public:
    ...
    // constructor copies the arguments into local storage
    vector_min_max(const std::vector<ScalarType>& vec) : v(vec), result_min(1e100), result_max(-1e100)
    {}

    // splitting constructor
    vector_min_max(vector_min_max& vdp, tbb::split) : v(vdp.v), result_min(1e100), result_max(-1e100)
    {}
...
//-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
template<class ScalarType, class VectorType>
class vector_min_max_magnitude
{
    const std::vector<ScalarType>& v;
public:
    ScalarType result_min;  // put the result here
    ...
    // constructor copies the arguments into local storage
    vector_min_max_magnitude(const std::vector<ScalarType>& vec) : v(vec), result_min(1e100), result_max(-1e100)
    {}

    // splitting constructor
    vector_min_max_magnitude(vector_min_max& vdp, tbb::split) : v(vdp.v), result_min(1e100), result_max(-1e100)
    {}
    ...
    void operator()(const tbb::blocked_range<size_t>& r)
    {
        for(size_t i = r.begin(), iEnd = r.end(); i != iEnd; i++)
        {
            ScalarType mag2;
#ifdef __Using_Eigen_Lib__
            mag2 = v[i].squaredNorm();
#else
#ifdef __Using_GLM_Lib__
            mag2 = glm::length2(v[i]);
#else             // use default yocto
            magx; // should throw error
#endif // __Using_GLM_Lib__
#endif            // __Using_Eigen_Lib__

            result_min = result_min < mag2 ? result_min : mag2;
            result_max = result_max > mag2 ? result_max : mag2;
        }
    }

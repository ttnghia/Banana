...
ShaderProgram* ShaderProgram::getSimpleDepthShader(std::string programName /*= std::string("SimpleDepthShader")*/)
{
    const GLchar* vertexShader =
        ...
        "\n"
        "in vec3 v_Position;\n"
        "out vec3 f_LightCoord;\n"
        "\n"
        "void main()\n"
        "{\n"
        "    vec4 lightCoord = lightView * modelMatrix * vec4(v_Position, 1.0);\n"
        "    f_LightCoord = lightCoord.xyz;\n"
        "    gl_Position = lightProjection * lightCoord;\n"
        "}";
    ...
    const GLchar* fragmentShader =
        ...
        "void main()\n"
        "{\n"
        "    vec4 pixelPos = vec4(f_LightCoord, 1.0f);\n"
        "    outDepth = f_LightCoord.z;\n"
        "    vec4 clipSpacePos = lightProjection * pixelPos;\n"
...
ShaderProgram* ShaderProgram::ShaderProgram::getPhongShader(std::string programName)
{
    ...
    const GLchar* fragmentShader =
        ...
        "uniform sampler2D u_TexSampler;\n"
        "uniform sampler2D m_UShadowMap;\n"
        "\n"
        "in VS_OUT\n"
        "{\n"
        "   vec3 f_Normal;\n"
